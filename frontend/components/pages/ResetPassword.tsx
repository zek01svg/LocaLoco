...
export function ResetPasswordPage({ email, token, onSuccess }: ResetPasswordPageProps) {
  ...
  const ValidationItem = ({ isValid, text }: { isValid: boolean; text: string }) => (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span style={{ color: isValid ? (isDarkMode ? '#4ade80' : '#16a34a') : mutedTextColor }}>
        {text}
      </span>
    </div>
  );
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-90">Discover and support local businesses in your community - or nearby you!</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="rounded-lg shadow-lg p-8 space-y-6" style={{ backgroundColor: cardBgColor, color: textColor }}>
            <div className="text-center space-y-2">
              ...
              <p className="text-sm" style={{ color: mutedTextColor }}>
                Create a new password for <span className="font-medium" style={{ color: textColor }}>{email}</span>
              </p>
            </div>
            ...
            {touched && password.length > 0 && (
              <div className="rounded-lg p-4 space-y-2" style={{ backgroundColor: validationBgColor }}>
                <p className="text-sm mb-2" style={{ color: textColor }}>Password must contain:</p>
                <ValidationItem isValid={validation.minLength} text="At least 8 characters" />
                <ValidationItem isValid={validation.hasUppercase} text="At least one uppercase letter (A-Z)" />
                <ValidationItem isValid={validation.hasLowercase} text="At least one lowercase letter (a-z)" />
                <ValidationItem isValid={validation.hasNumber} text="At least one number (0-9)" />
                <ValidationItem isValid={validation.hasSymbol} text="At least one symbol (!@#$%^&*...)" />
                {confirmPassword.length > 0 && (
                  <ValidationItem isValid={validation.passwordsMatch} text="Passwords match" />
                )}
              </div>
            )}

            <Button type="submit" className="w-full bg-primary hover:bg-primary/90" disabled={isSubmitting || (touched && !isValid)}>
