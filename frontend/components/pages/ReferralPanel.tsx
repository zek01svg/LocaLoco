...
export default function ReferralPanel({ userId, appBaseUrl = window.location.origin }: Props) {
  ...
  return (
    <div className="rounded-2xl border p-4 space-y-3">
      <div className="text-lg font-semibold">Invite friends · Get rewards</div>
      <div className="text-sm">
        Your code: <b>{code}</b>
      </div>
      <div className="flex gap-2">
        <input className="border rounded p-2 flex-1" value={shareUrl} readOnly />
        <button
      ...
      <div className="text-sm">
        Successful referrals: <b>{count}</b><br/>
        Vouchers earned (total): <b>${totalAmt.toFixed(2)}</b>
      </div>
      <p className="text-xs text-gray-600">
        Rewards: Earn a $5 voucher by referring a friend! Refer more than 5 friends and get a $10 voucher!
      </p>
    </div>
  );
}
