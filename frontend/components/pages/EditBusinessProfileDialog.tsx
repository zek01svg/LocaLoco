...
export function EditBusinessProfileDialog({
  ...
  const normalizePaymentOptions = (options: string[]): string[] => {
    ...
    options.forEach(option => {
      ...
      if (lowercased === 'cash' || lowercased === 'Cash') {
        normalized.add('cash');
      } else if (lowercased === 'card' || lowercased === 'credit/debit card') {
        normalized.add('card');
      } else if (lowercased === 'paynow') {
        normalized.add('paynow');
      } else if (lowercased === 'digital_wallets' || lowercased === 'digital wallets') {
  ...
  useEffect(() => {
    setFormData({
      ...
      paymentOptions: normalizePaymentOptions(businessOwner.paymentOptions || [])
    });
  }, [businessOwner]);
  ...
  const handleDayToggle = (day: string) => {
    ...
    if (isDayCurrentlySelected) {
      ...
      const newOperatingDays = updatedOperatingDays.filter(d => d !== day);
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-20 h-20 rounded-lg flex items-center justify-center overflow-hidden bg-gray-200">
                      {previewUrl ? <img src={previewUrl} alt="Business preview" className="w-full h-full object-cover" /> : <Upload className="w-8 h-8 text-gray-400" />}
                    </div>
                    <div className="flex gap-2">
                      <Button type="button" variant="outline" size="sm" onClick={triggerFileInput} disabled={uploading}>
                        {uploading ? 'Uploading...' : previewUrl ? 'Change' : 'Upload'}
                      </Button>
                      {previewUrl && <Button type="button" variant="outline" size="sm" onClick={handleRemoveWallpaper}><X className="w-4 h-4" /></Button>}
                    </div>
                    <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                  </div>
                  <p className="text-xs text-muted-foreground">Recommended: Landscape image, max 5MB</p>
                </div>
                <div className="space-y-2">
            ...
            return (
                <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center space-x-2">
                            <Checkbox id="offersDelivery" checked={formData.offersDelivery} onCheckedChange={(checked: boolean) => handleChange('offersDelivery', checked)} />
                            <Truck className="w-4 h-4 text-muted-foreground" />
                            <label htmlFor="offersDelivery" className="text-sm">Offers Delivery</label>
                        </div>
                        <div className="flex items-center space-x-2">
                            <Checkbox id="offersPickup" checked={formData.offersPickup} onCheckedChange={(checked: boolean) => handleChange('offersPickup', checked)} />
                            <ShoppingBag className="w-4 h-4 text-muted-foreground" />
                            <label htmlFor="offersPickup" className="text-sm">Offers Pickup</label>
                        </div>
                    </div>
                    <Separator />
                    <div className="space-y-3">
                     ...
                     <div className="space-y-3">
                        ...
                        <div className="border border-input rounded-md p-4 grid grid-cols-2 gap-4">
                            {PAYMENT_OPTIONS.map(payment => (
                                ...
                                <Checkbox id={`edit-payment-${payment.value}`} checked={(formData.paymentOptions || []).includes(payment.value)} onCheckedChange={() => handlePaymentToggle(payment.value)} />
                                <label htmlFor={`edit-payment-${payment.value}`} className="text-sm">{payment.label}</label>
                                </div>
                            ))}
                        </div>
                    </div>
          ...
          case 4:
            const getPaymentLabels = (values: string[]): string => {
              return values.map(val => {
                const option = PAYMENT_OPTIONS.find(opt => opt.value === val);
            ...
            return (
                <div className="space-y-4">
                    ...
                    <div className="space-y-2 text-sm max-h-80 overflow-y-auto">
                        ...
                        <p><strong>Price Tier:</strong> {formData.priceTier}</p>
                        <p><strong>Payment Options:</strong> {getPaymentLabels(formData.paymentOptions || [])}</p>
                        <Separator className="my-2"/>
                        <p><strong>Offers Delivery:</strong> {formData.offersDelivery ? 'Yes' : 'No'}</p>
                        <p><strong>Offers Pickup:</strong> {formData.offersPickup ? 'Yes' : 'No'}</p>
                    </div>
                </div>
            );
          default:
      ...
      >
        ...
        >
              <div className="flex justify-between w-full">
                  {currentStep > 1 && (<Button type="button" variant="outline" onClick={handleBack}><ChevronLeft className="h-4 w-4 mr-2" />Previous</Button>)}
                  <div className="flex-grow"></div>
                  {currentStep < TOTAL_STEPS && (<Button type="button" onClick={handleNext}>Next<ChevronRight className="h-4 w-4 ml-2" /></Button>)}
                  {currentStep === TOTAL_STEPS && (<Button type="submit" form="edit-business-form" disabled={saving}>{saving ? 'Saving...' : 'Save Changes'}</Button>)}
              </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
